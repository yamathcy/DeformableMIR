...
class LocalAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        print("local_attention_input {}".format(x.shape))
...
class DAT(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.patch_proj(x)
        print("proj: {}".format(x.shape))
        positions = []
        references = []
        for i in range(4):
            x, pos, ref = self.stages[i](x)
            print("stages: {}".format(x.shape))
            if i < 3:
                x = self.down_projs[i](x)
                print("down_proj: {}".format(x.shape))
            positions.append(pos)
            references.append(ref)
        x = self.cls_norm(x)
...
class SpecDAT(nn.Module):

    def __init__(self, img_size=(128, 300), patch_size=4, num_classes=1000, expansion=4,
                 dim_stem=96, dims=[96, 192, 384, 768], depths=[2, 2, 6, 2], 
                 heads=[3, 6, 12, 24], 
        ...
        # img_size = img_size // patch_size
        freq_size = img_size[0]//patch_size
        time_size = img_size[1]//patch_size
        img_size = (freq_size,time_size)
        ...
        for i in range(4):
            ...
            img_size = [size // 2 for size in img_size]

...
"""
Radial attention transformer by Chen et al, ICASSP 2023
"""
class RATBlock(nn.Module):
    def __init__(
        ...
        dilations_s = [2 for x in range(depth)]
        self.dina_s = nn.ModuleList(
            [
                NATLayer(
                    dim=dim,
                    num_heads=num_heads,
                    kernel_size=kernel_size,
                    dilation=dilations_s[i],
                    mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
                    drop=drop,
        ...
        dilations_m = [3 for x in range(depth)]
        self.dina_m = nn.ModuleList(
            [
                NATLayer(
                    dim=dim,
                    num_heads=num_heads,
                    kernel_size=kernel_size,
                    dilation=dilations_m[i],
                    mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
                    drop=drop,
